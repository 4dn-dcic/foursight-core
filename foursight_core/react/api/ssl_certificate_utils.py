...
def get_ssl_certificate_info(hostname: str, raise_exception: bool = False) -> Optional[dict]:
    """
    ...
    try:
        ...
        certificate_info = _get_ssl_certificate_info_from_pem(certificate_pem, raise_exception=raise_exception)
...
def _get_ssl_certificate_info_from_pem(pem_string: str, raise_exception: bool = False) -> Optional[dict]:
    """
    ...
    def is_datetime_within_ndays(d: datetime, ndays: int) -> bool:
        """
        Returns True iff the given datatime is within "ndays" days (in the past) of the current
        datetime, i.e. if the given datetime plus "ndays" days is greater-than-or-equal to the
        current datetime; otherwise returns False.
        """
        return (now - d) <= timedelta(days=ndays) if d <= now else False
    ...
    try:
        ...
        expires_soon = is_datetime_within_ndays(expires_at, _SSL_CERTIFICATE_EXPIRES_SOON_WARNING_DAYS)
