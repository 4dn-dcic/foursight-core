...
import Styles from '../Styles';
    ...
    function hideGroup(group, groupList, historyList) {
        ...
        group.__selectedXyzzy = false;
        group.__selectedOnceXyzzy = true;
    ...
    const SelectedGroupsPanel = ({ groupList, env, historyList, info, checks }) => {
        ...
        return <>
            ...
            { checks.map((selectedGroup, index) =>
                <SelectedGroupBox key={selectedGroup.group} group={selectedGroup} env={env} groupList={groupList} historyList={historyList} info={info} style={{paddingBottom:selectedGroup.__selectedXyzzy ? "6pt" : "0"}} />
            )}
        </>
                    /*
        return <div>
        ...
        return <>
            { group?.checks?.sort((a,b) => a.title > b.title ? 1 : (a.title < b.title ? -1 : 0)).map((check, index) =>
                <div key={index}>
                    ...
                    <SelectedGroupCheckBox check={check} env={env} groupList={groupList} historyList={historyList} info={info} group={group} />
                </div>
            )}
        </>
        */
        return <div style={style}>
            <div className="box" style={{display:!group.__selectedXyzzy ? "none" : "inherit",paddingTop:"6pt",paddingBottom:"6pt",marginBottom:"4pt",minWidth:"300pt"}}>
    ...
    const SelectedGroupCheckBox = ({check, env, groupList, historyList, info, group }) => {

        const runActionAllowedState = useState(false);

        let result = useFetch();
        let resultByUuid = useFetch();
        let resultByAction = useFetch();
...
//      }, []);
        ...
        useEffect(() => {
            if (group.__selectedXyzzy && !group.__selectedOnceXyzzy) {
                check.__showingResults = true;
                refreshResults(check, env, groupList);
            }
        }, [group.__selectedXyzzy]);
        ...
        if (!group.__selectedXyzzy) return null;
        return <div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style={{veriticalAlign:"top",maxWidth:"600pt",width:"100%",whiteSpace:"nowrap"}}>
                            ...
                            <RunActionBox check={check} env={env} groupList={groupList} update={() => groupList.update()} refreshResults={refreshResults} runActionAllowedState={runActionAllowedState} />
                            <ActionRunningBox check={check} groupList={groupList} info={info} />
                            <ResultBox check={check} env={env} groupList={groupList} refreshResults={refreshResults} runActionAllowedState={runActionAllowedState} />
                        </td>
                    </tr>
                    <tr style={{height:"3pt"}}><td></td></tr>
                </tbody>
    ...
    const ResultBox = ({ check, env, groupList, refreshResults, runActionAllowedState }) => {
...
const ChecksPage = (props) => {
    ...
    const ChecksGroupBox = ({props}) => {
        return <div style={{minWidth:"150pt"}}>
            <div style={{fontWeight:"bold",paddingBottom:"3pt"}}>Check Groups</div>
            ...
            <table><tbody>
                <tr>
                    ...
                    <td style={{paddingLeft:"10pt",verticalAlign:"top"}}>
                        ...
                        <SelectedGroupsPanel env={environ} groupList={groupList} historyList={historyList} info={info} checks={checks} />
                    </td>
...
const RunActionBox = ({ check, env, groupList, refreshResults, runActionAllowedState }) => {
