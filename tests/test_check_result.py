...
class TestCheckResult:
    ...
    def test_check_result_methods(self, use_es, app_utils_obj_conn):
        ...
        assert (res['kwargs']['primary'] is False)
        ...
        res_uuid = res['uuid']
        check_copy = run_result.CheckResult(connection, self.check_name, init_uuid=res_uuid)
        if not use_es:
    ...
    @pytest.mark.parametrize('use_es', [False])
    def test_get_closest_result(self, use_es, app_utils_obj_conn):
        """ Enabling the ES layer causes test flakiness due to the index refresh interval.
            Presumably the test would pass for ES if you inserted time.sleep(2) after every call
            to store_result.
        """
        ...
        bad_check = run_result.CheckResult(connection, 'not_a_real_check')
        if not use_es:
        ...
        error_check = run_result.CheckResult(connection, self.error_check_name)
    ...
    def test_get_result_history(self, use_es, app_utils_obj_conn):
        """
        ...
        if check.connections['es'] is not None:
            check.connections['es'].refresh_index()
        hist_10 = check.get_result_history(0, 10)[0]
        ...
        if len(hist_10) != 10:
            return
        hist_offset = check.get_result_history(1, 2)[0]
        hist_offset2 = check.get_result_history(2, 2)[0]
        ...
        hist_after = check.get_result_history(0, 10, after_date=after_date)[0]
        for chk in hist_after:
            chk_date = datetime.datetime.strptime(chk[2]['uuid'], '%Y-%m-%dT%H:%M:%S.%f')
            assert chk_date >= after_date

    def test_filename_to_datetime(self, app_utils_obj_conn):
        _, connection = app_utils_obj_conn
        check = run_result.CheckResult(connection, self.check_name)
